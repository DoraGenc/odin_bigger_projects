...
RSpec.describe LinkedList do
  ...
  describe '#insert_at' do
    context 'when entering an invalid index' do
      it 'returns an error' do
        invalid_index = -3
        value = 'a'
        expect(linked_list.insert_at(invalid_index,
                                     value)).to eq('The index is invalid. Please only enter integers starting from 0.')
      end
    end
